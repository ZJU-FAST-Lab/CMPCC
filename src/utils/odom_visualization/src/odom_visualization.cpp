...
odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  if (msg->header.frame_id == string("null"))
    return;
  // jljl
  Quaterniond q_odom(msg->pose.pose.orientation.w, msg->pose.pose.orientation.x, msg->pose.pose.orientation.y, msg->pose.pose.orientation.z);
  Quaterniond q_odom_loop(w_R_odom * (q_odom.toRotationMatrix()));
  Vector3d pDrone( msg->pose.pose.position.x, msg->pose.pose.position.y, msg->pose.pose.position.z);
  Vector3d vDrone( msg->twist.twist.linear.x, msg->twist.twist.linear.y, msg->twist.twist.linear.z);
  Vector3d aDrone( msg->twist.twist.angular.x, msg->twist.twist.angular.y, msg->twist.twist.angular.z);
  pDrone= w_R_odom * pDrone + w_t_odom;
  vDrone = w_R_odom * vDrone;
  aDrone = w_R_odom * aDrone;
  nav_msgs::Odometry msg233 = *msg;
  msg233.pose.pose.position.x = pDrone(0);
  msg233.pose.pose.position.y = pDrone(1);
  msg233.pose.pose.position.z = pDrone(2);
  msg233.twist.twist.linear.x = vDrone(0);
  msg233.twist.twist.linear.y = vDrone(1);
  msg233.twist.twist.linear.z = vDrone(2);
  msg233.twist.twist.angular.x = aDrone(0);
  msg233.twist.twist.angular.y = aDrone(1);
  msg233.twist.twist.angular.z = aDrone(2);
  msg233.pose.pose.orientation.w = q_odom_loop.w();
  msg233.pose.pose.orientation.x = q_odom_loop.x();
  msg233.pose.pose.orientation.y = q_odom_loop.y();
  msg233.pose.pose.orientation.z = q_odom_loop.z();
  ...
  colvec pose(6);
  pose(0) = msg233.pose.pose.position.x;
  pose(1) = msg233.pose.pose.position.y;
  pose(2) = msg233.pose.pose.position.z;

  colvec q(4);
  q(0) = msg233.pose.pose.orientation.w;
  q(1) = msg233.pose.pose.orientation.x;
  q(2) = msg233.pose.pose.orientation.y;
  q(3) = msg233.pose.pose.orientation.z;
  pose.rows(3, 5) = R_to_ypr(quaternion_to_R(q));

  Eigen::Quaterniond quaterniond;
  quaterniond.w() = msg233.pose.pose.orientation.w;
  quaterniond.x() = msg233.pose.pose.orientation.x;
  quaterniond.y() = msg233.pose.pose.orientation.y;
  quaterniond.z() = msg233.pose.pose.orientation.z;
  ...
  colvec vel(3);
  vel(0) = msg233.twist.twist.linear.x;
  vel(1) = msg233.twist.twist.linear.y;
  vel(2) = msg233.twist.twist.linear.z;
  ...
  // Pose
  poseROS.header       = msg233.header;
  poseROS.header.stamp = msg233.header.stamp;
  ...
  // string("/local_map_") + std::to_string(::count);
  velROS.header.stamp       = msg233.header.stamp;
  ...
  // Path
  static ros::Time prevt = msg233.header.stamp;
  if ((msg233.header.stamp - prevt).toSec() > 0.1)
  {
    prevt          = msg233.header.stamp;
  ...
  double g = 1;
  double b = 1;
  bool   G = msg233.twist.covariance[33];
  bool   V = msg233.twist.covariance[34];
  bool   L = msg233.twist.covariance[35];
  if (cov_color)
  {
    r = G;
    g = V;
  ...
  if (cov_pos)
  {
    ...
    for (int j = 0; j < 6; j++)
      for (int i = 0; i < 6; i++)
        P(i, j) = msg233.pose.covariance[i + j * 6];
    colvec eigVal;
    mat    eigVec;
    ...
    // string("/local_map_") + std::to_string(::count);
    covROS.header.stamp       = msg233.header.stamp;
  ...
  if (cov_vel)
  {
    ...
    for (int j = 0; j < 3; j++)
      for (int i = 0; i < 3; i++)
        P(i, j) = msg233.twist.covariance[i + j * 6];
    ...
    // string("/local_map_") + std::to_string(::count);
    covVelROS.header.stamp       = msg233.header.stamp;
  ...
  static ros::Time pt    = msg233.header.stamp;
  ros::Time        t     = msg233.header.stamp;
  if ((t - pt).toSec() > 0.5)
  {
  ...
  // string("/local_map_") + std::to_string(::count);
  sensorROS.header.stamp       = msg233.header.stamp;
  ...
  double H                  = msg233.twist.covariance[32];
  heightROS.header.frame_id = string("height");
  heightROS.header.stamp    = msg233.header.stamp;
  ...
  // string("/local_map_") + std::to_string(::count);
  meshROS.header.stamp = msg233.header.stamp;
  meshROS.ns           = "drone";
  ...
  {
    meshROS.pose.position = msg233.pose.pose.position;
    q(0)                  = msg233.pose.pose.orientation.w;
    q(1)                  = msg233.pose.pose.orientation.x;
    q(2)                  = msg233.pose.pose.orientation.y;
    q(3)                  = msg233.pose.pose.orientation.z;
  ...
  if (tf45)
  {
    ...
    broadcaster->sendTransform(tf::StampedTransform(
      transform, msg233.header.stamp, target_frame, selfstr));
    //    broadcaster->sendTransform(tf::StampedTransform(
    //      transform45, msg233.header.stamp, selfstr, string("/laser")));
    //    broadcaster->sendTransform(tf::StampedTransform(
    //      transform45, msg233.header.stamp, selfstr, string("/vision")));
    //    broadcaster->sendTransform(tf::StampedTransform(
    //      transform90, msg233.header.stamp, selfstr, string("/height")));
  }
  ...
  if (tfc && (target_frame != baseFrame))
  {
    ...
    broadcaster->sendTransform(
      tf::StampedTransform(transform, msg233.header.stamp, baseFrame,
